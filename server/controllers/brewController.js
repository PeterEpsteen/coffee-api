...
function getBrews(req, res, next) {
    ...
    let sort = (req.query.hasOwnProperty("sort")) ? req.query.sortBy : "points";
    req.query.sort = sort;
    db.any("(SELECT b.*, u.username FROM (SELECT b.*, COUNT(c.brew_id) AS comments FROM brew AS b LEFT JOIN brew_comment AS c ON c.brew_id = b.brew_id GROUP BY b.brew_id) AS b INNER JOIN users as u ON b.user_id = u.id ${filter_method}) ORDER BY b.${sort} DESC OFFSET ${offset} LIMIT ${limit}", req.query)
        .then(data => {
            console.log(data);
            res.status(200).json({
                status:'success',
