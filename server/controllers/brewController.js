...
function getBrews(req, res, next) {
    ...
    req.query.search = (req.query.hasOwnProperty("search")) ?
                        `%${req.query.search}%` : "%%"; 
    ...
    db.any("(SELECT b.*, u.username FROM (SELECT b.*, COUNT(c.brew_id) AS comments FROM brew AS b LEFT JOIN brew_comment AS c ON c.brew_id = b.brew_id GROUP BY b.brew_id) AS b INNER JOIN users as u ON b.user_id = u.id WHERE b.brew_method LIKE ${filter_method} AND brew_name LIKE ${search}) ORDER BY ${sort^} ${order^} OFFSET ${offset} LIMIT ${limit}", req.query)
        .then(data => {
            res.status(200).json({
                status:'success',
                data: data,
